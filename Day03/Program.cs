...
var groups = new List<ValueTuple<List<char>, List<char>, List<char>>>();

for (var i = 0; i < sacks.Count; i += 3)
{
    groups.Add(new ValueTuple<List<char>, List<char>, List<char>>(sacks[i], sacks[i + 1], sacks[i + 2]));
}

var result = groups.Select(FindCommon)
    .Select(CharValue)
    .Sum();
...
{
    var offset = char.IsLower(character) ? 96 : 38;
    var asciiCode = (int)character;

    return asciiCode - offset;
}

char FindCommon(ValueTuple<List<char>, List<char>, List<char>> group)
{
    return group.Item1.Intersect(group.Item2).Intersect(group.Item3).FirstOrDefault();
}